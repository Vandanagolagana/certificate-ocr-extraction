...
try:
    model = joblib.load("tenth_classifier.pkl")
    ...
    for file_path in file_paths:
        ...
        if not os.path.exists(file_path):
            ...
            try:
                ...
                # Use ML model if regex extraction fails
                if model:
                    for attr in attributes:
                        if attr not in extracted_attributes or extracted_attributes[attr] == "Not Found":
...
def extract_attributes(text, attributes):
    extracted_info = {}
    patterns = {
        "name": r"CERTIFIED THAT\s+([A-Za-z.'-]+\s+[A-Za-z.'-]+)",  
        "father name": r"FATHER NAME\s*[:\-]?\s*([A-Za-z.'-]+\s+[A-Za-z.'-]+)",  
        "mother name": r"MOTHER NAME\s*[:\-]?\s*([A-Za-z.'-]+\s+[A-Za-z.'-]+)",  
        "roll number": r"bearing Roll No\.?\s*(\d{10})",  
        "school name": r"belonging to\s*([A-Za-z\s.,'-]+)",  
        "dob": r"DATE OF BIRTH\s*[:\-]?\s*(\d{2}/\d{2}/\d{4})",  
        "gpa": r"\b(?:10(?:\.0)?|([0-9]\.\d{1,2}))\b"  
    }
    ...
    for attribute in attributes:
        ...
        if pattern:
            ...
            if match:
                ...
                extracted_info[attribute] = extracted_value.strip()
            else:
...
def predict_attribute(text):
    """
    Uses the ML model to predict missing attributes.
